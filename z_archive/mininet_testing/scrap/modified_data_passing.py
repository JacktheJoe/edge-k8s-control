...
def record_leader_decision(node_name, decision):
    decision_file_path = f"/home/jack/Desktop/mininet/RAFT-like/results/leader_decision/{node_name}.txt"
    with open(decision_file_path, 'w') as file:
        file.write(decision)
...
# Define the paths as in the original RAFT.py script
...
def save_local_record(node_name):
    ...
    sorted_metrics = sorted(neighbor_metrics.items(), key=lambda x: x[0])  # Sort by node name

    # Extract and sort the node's own metric
    own_metrics = {
        "Node": node_name,
        "Cpu": cpu_usage,
        "Ram": ram_usage,
    ...
    with open(csv_file, mode='w', newline='') as file:
        ...
        for neighbor, metrics in sorted_metrics:
            writer.writerow([neighbor, metrics["Cpu"], metrics["Ram"], metrics["Latency"], metrics["Bandwidth"]])

        # Add the node's own metric record
        writer.writerow([own_metrics["Node"], own_metrics["Cpu"], own_metrics["Ram"], own_metrics["Latency"], own_metrics["Bandwidth"]])
...
def write_lowest_utilization_to_file(node_name):
    output_file = os.path.join(results_path, "results_node", f"{node_name}.txt")

    with open(output_file, mode='w') as file:
        file.write(f"Lowest Utilization Node: {lowest_node_name}\n")
        file.write(f"Lowest Utilization Value: {lowest_utilization}")
...
# Placeholder for handling the leader's decision
# This should be replaced with actual logic to handle the reception of the leader's decision
# Example:
# if is_leader_decision_received():
#     decision = get_leader_decision()
#     record_leader_decision(node_name, decision)
