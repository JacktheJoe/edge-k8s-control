...
log_file_location="/code/bash_logs.txt"
network_logs_location="/code/network_logs.csv"
...
send_json_post(){
	...
	echo "\n$current_time\n" >> $log_file_location
	echo "$send_data" >> $log_file_location
...
prep(){
	...
	if [! -f "$network_logs_location" ]; then
		touch "$network_logs_location"
	fi
