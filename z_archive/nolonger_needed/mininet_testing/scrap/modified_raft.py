...
class Node(object):
    def fetch_lowest_util_node(self):
        files_path = "/home/jack/Desktop/mininet/RAFT-like/results/results_node/*.txt"
        lowest_util_node = None
        lowest_util_value = float('inf')

        for file_path in glob.glob(files_path):
            with open(file_path, 'r') as file:
                node_name = os.path.basename(file_path).replace('.txt', '')
                util_value = float(file.read().strip())
                if util_value < lowest_util_value:
                    lowest_util_value = util_value
                    lowest_util_node = node_name

        return lowest_util_node
