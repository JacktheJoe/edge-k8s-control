...
prep(){
    ...
    [[ ! -f $LOCAL_METRIC_LOG ]] && touch "$LOCAL_METRIC_LOG"
    [[ ! -f $DECISION_LOG ]] && touch "$DECISION_LOG"
    # keep just in case
    [[ ! -f $REDEPLOYMENT_LOG ]] && touch "$REDEPLOYMENT_LOG"
...
startreceivingdeployment(){
    uvicron deployment_request:app --reload --host $NODE_IP --port $PEER_DEPLOYMENT_PORT &
}
...
# start decision
if ! startdecision; then
    printf "Failed to start decision\n" >&2
    exit 1
fi
